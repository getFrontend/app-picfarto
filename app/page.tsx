...
import { cutImageIntoGrid, cutImageWithGridLines, createAndDownloadZip } from '@/utils/imageProcessing';
...
export default function Home() {
  ...
  const handleGridChange = useCallback((newRows: number, newColumns: number) => {
    setRows(newRows);
  ...
  const handleClientSideCut = useCallback(async () => {
    ...
    try {
      ...
      if (gridLines.length > 0 && imageSize.width > 0 && imageSize.height > 0) {
        images = await cutImageWithGridLines(imageUrl, gridLines, imageSize.width, imageSize.height);
      } else {
  ...
  const handleServerSideCut = useCallback(async () => {
    ...
    try {
      ...
      alert('Failed to process image on server. Falling back to client-side processing...');
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h1 className="text-3xl font-bold mb-2">Image Grid Cutter</h1>
